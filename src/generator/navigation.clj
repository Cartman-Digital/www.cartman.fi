  ...
  (let [menu-data (contentful/get-contentful :nav-collection-query {:where {:name top-nav}})
    ...
    [:nav {:class "border-gray-800 border-b"} 
     [:div {:class "max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4"}
      ...
      [:button {:data-collapse-toggle "navbar-default" :type "button" :class "inline-flex items-center p-2 ml-3 text-sm rounded-lg md:hidden focus:outline-none focus:ring-2 focus:ring-gray-200 text-gray-400 hover:bg-gray-700 focus:ring-gray-600" :aria-controls "navbar-default" :aria-expanded "false"}
